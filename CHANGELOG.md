...
## v2.11.0
- 은행별 하드코딩 대신 사용자가 직접 저장하는 `은행 내역 템플릿` 도입
- 새 템플릿 생성 흐름: 샘플 파일 → 헤더 행 → 열 매핑 → 5행 미리보기 → 자유 이름 → 저장. 헤더만 있는 빈 샘플은 템플릿만 저장하고 첫 실제 파일에서 미리보기 확인
- CSV/TSV 및 Excel 97-2003 `.xls` import 지원
- 입금 날짜/시간 분리 열, 입금자명, 입금액, 은행 고유번호(선택) 매핑 지원
- 저장된 템플릿은 IndexedDB settings 및 JSON 백업/복원에 포함
- 외부 sheet 파일만 참조하는 HTML `.xls`는 데이터 부재를 명확히 안내
