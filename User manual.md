# ClassRelay v2.11.0 — User Manual
...
4. 은행 내역 파일을 가져옵니다. 처음 쓰는 형식은 샘플 파일에서 헤더/열을 지정해 템플릿으로 저장하고, 다음부터는 저장한 템플릿으로 바로 가져와 신규 입금을 누적하고 자동매칭합니다.
...
- 이후 강의 가격을 바꿔도 기존 신청 건의 저장 금액은 바뀌지 않으며, 은행 입금내역의 실제 입금액은 그 신청 당시 금액과 비교합니다.
- 템플릿 제목 또는 본문에 `{{녹화본URL}}`이 들어 있는 경우에만 해당 강의에 녹화본 URL이 없으면 발송을 막습니다.


## 은행 내역 템플릿
- 처음 사용하는 파일 형식은 `새 템플릿 만들기`에서 샘플 파일을 올립니다.
- 실제 입금 표가 시작하는 `헤더 행`을 지정합니다.
- `입금 날짜`, `입금 시간(선택)`, `입금자명`, `입금액`, `은행 고유번호(선택)` 열을 연결합니다.
- 실제 입금 행이 있으면 정규화된 5행 미리보기를 확인한 뒤 `KB 개인계좌`처럼 원하는 이름으로 저장합니다. 헤더만 있는 빈 샘플도 템플릿 저장은 가능하며, 첫 실제 입금 파일에서 5행 미리보기를 한 번 확인합니다.
- 다음부터는 저장된 템플릿을 선택하고 새 입금내역 파일만 업로드하면 됩니다. 파일 구조가 저장 시점과 달라지면 다시 확인하도록 안내합니다.
- CSV/TSV와 Excel 97-2003 `.xls`를 지원합니다. 확장자가 `.xls`이지만 파일 안에는 표가 없고 외부 `*.files/sheet001.htm`만 참조하는 HTML 파일은 데이터가 파일 자체에 없으므로 가져올 수 없다는 안내를 표시합니다.
- 파일은 브라우저에서만 읽고 서버로 업로드하지 않습니다. 은행 내역 템플릿도 브라우저 IndexedDB에 저장되며 JSON 백업/복원에 포함됩니다.
