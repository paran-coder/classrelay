# ClassRelay v2.10.0 — User Manual
...
## 강의 운영
1. 강의 관리에서 강의명과 가격을 저장합니다. 녹화본 URL은 선택사항이며 나중에 추가할 수 있습니다.
2. 해당 강의의 Google Form 긴 편집 `/edit` URL을 연결합니다.
3. 전체 폼 동기화 또는 이 강의 폼 동기화로 신청자를 가져옵니다.
4. 은행 CSV를 가져오면 신규 입금을 누적하고 즉시 자동매칭합니다.
5. 메일 템플릿을 확인합니다. 일반 안내메일은 녹화본 URL 없이 보낼 수 있고, `{{녹화본URL}}` 변수를 쓰는 템플릿만 강의의 녹화본 URL이 필요합니다.
...
## v2.10.0 강의·Form·메일 운영 규칙
- 강의의 `녹화본 URL`은 선택값입니다. 신청 접수와 입금 확인, 일반 안내메일 발송에는 필요하지 않습니다.
- Google Form에는 신청자 이름, 입금자명, 이메일, 연락처 등 신청자 정보만 받습니다. `결제금액`을 사용자에게 입력받지 않습니다.
- 신청이 처음 동기화될 때 ClassRelay가 연결된 강의의 현재 가격을 신청 금액으로 저장합니다.
- 이후 강의 가격을 바꿔도 기존 신청 건의 저장 금액은 바뀌지 않으며, 은행 CSV의 실제 입금액은 그 신청 당시 금액과 비교합니다.
- 템플릿 제목 또는 본문에 `{{녹화본URL}}`이 들어 있는 경우에만 해당 강의에 녹화본 URL이 없으면 발송을 막습니다.
