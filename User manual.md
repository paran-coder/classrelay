...
- `+ 템플릿 추가`: 새 템플릿 생성
- `기본 템플릿으로 지정`: 강의에 전용 템플릿이 없을 때 사용할 fallback 지정
- `사용 강의`: 현재 템플릿을 특정 강의에 연결
- `복제`: 기존 템플릿을 복사해 변형
- `삭제`: 기본 템플릿이 아닌 템플릿 삭제
- `미리보기`: 변수 치환 결과 확인
...
템플릿은 별도 IndexedDB store를 만들지 않고 기존 `settings` store 안에 저장합니다. 과거 v2.6.0의 실험적 `templates` store가 브라우저에 남아 있으면 v2.6.1이 내용을 settings 구조로 복구합니다.
