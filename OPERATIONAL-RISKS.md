# ClassRelay v2.8.1 — Operational Risk Status

## 데이터/발송 로직
v2.8.1은 운영 로직을 변경하지 않는 UI·가이드 패치입니다. v2.8.0에서 해결한 다음 보호장치를 그대로 유지합니다.
...
## v2.8.1 변경으로 새로 생긴 운영 위험
현재 확인된 신규 운영 위험은 없습니다. 강의 추가 모달 CSS와 문서만 변경했고 IndexedDB schema, 매칭, Gmail 전송 경로는 변경하지 않았습니다.
